...
int UnitTest_Stack(void)
{
	...
	testStack.Push(&testStack, (testInteger));
	testStack.Push(&testStack, (testInteger+1));
	testStack.Push(&testStack, (testInteger+2));
	testStack.Push(&testStack, (testInteger+3));
	testStack.Push(&testStack, (testInteger+4));
	...
	if (testStack.Pop(&testStack, &popOutput) != &testStack){
		UNIT_TEST_FAIL;
		return -4;
	}

	if (popOutput != (testInteger + 4)){
		UNIT_TEST_FAIL;
		return -5;
	}
	...
	if (popOutput != (testInteger)){
		UNIT_TEST_FAIL;
		return -13;
	}

	if (testStack.Pop(&testStack, &popOutput) != NULL){
		UNIT_TEST_FAIL;
		return -14;
	}
