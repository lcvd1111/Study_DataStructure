...
int UnitTest_MinHeapStablePriorityQueue(void)
{
	HEAP testHeap;
	HEAP_NODE *tempArray = NULL;
	HEAP_NODE outputStore[30];
	memset(outputStore, 0, sizeof(HEAP_NODE)*30);

	if (Constructor_Heap(&testHeap, 10) != &testHeap){
		UNIT_TEST_FAIL;
		return -1;
	}
	...
	testHeap.Enqueue(&testHeap, 80, "b");
	testHeap.Enqueue(&testHeap, 30, "c");
	testHeap.Enqueue(&testHeap, 70, "d");
	testHeap.Enqueue(&testHeap, 80, "e");
	testHeap.Enqueue(&testHeap, 90, "f");
	testHeap.Enqueue(&testHeap, 50, "g");
	testHeap.Enqueue(&testHeap, 30, "h");
	testHeap.Enqueue(&testHeap, 40, "i");
	testHeap.Enqueue(&testHeap, 80, "j");
	testHeap.Enqueue(&testHeap, 100, "k");
	testHeap.Enqueue(&testHeap, 50, "l");
	testHeap.Enqueue(&testHeap, 30, "m");
	testHeap.Enqueue(&testHeap, 30, "n");
	testHeap.Enqueue(&testHeap, 60, "o");
	testHeap.Enqueue(&testHeap, 70, "p");
	testHeap.Enqueue(&testHeap, 50, "q");
	...
	for (int i=0 ; i<17 ; i++){
		if (testHeap.Dequeue_P(&testHeap, outputStore+i) != &testHeap){
			UNIT_TEST_FAIL;
			return -25;
		}
	}
	...
	if (Destructor_Heap(&testHeap) != &testHeap){
		UNIT_TEST_FAIL;
		return -9999;
	}
