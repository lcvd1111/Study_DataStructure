...
QUEUE *QUEUE_METHOD_Create(QUEUE *this, int sizeArg)
{
	...
	(*this).size = sizeArg;
	(*this).queueArray = (void **)malloc(sizeof(void *)*sizeArg);
	(*this).beginIndex = -1;
	(*this).endIndex = -1;
...
QUEUE *QUEUE_METHOD_Enqueue(QUEUE *this, void *inputArg)
{
	...
	//Exception Handling3
	if (inputArg == NULL){
		DEBUG("ERROR: inputArg is NULL. NULL value can't be enqueued.\n");
		return NULL;
	}
	...
	//When the current endIndex is not hte last element of the array.
	(*this).endIndex += 1;
	(*this).queueArray[(*this).endIndex] = inputArg;
